...
import { abbreviationCharacter, verticalLayout, inputModeCustomTranslations } from './config/config';
...
const Settings: React.FC<{closeNav, theme, setTheme}> =
  ...
  const [aChar, setAbbreviationCharacter] = React.useState(abbreviationCharacter)
  ...
  useEffect(() => {
    let _aChar= window.localStorage.getItem("abbreviationCharacter")
    ...
    if (_aChar) {
      setAbbreviationCharacter(_aChar)
